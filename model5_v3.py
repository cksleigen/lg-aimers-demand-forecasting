...
def predict_one_file_generic(
    ...
    if len(dates) < Lx:
        ...
        if len(dates) > 0:
            # 일부만 있는 경우: 앞쪽을 평균값으로 패딩 + 실제 시작일 기준으로 날짜 backfill
            k = min(7, len(dates))
            ...
            # 완전히 비어있는 경우: 학습 말단일을 앵커로 사용해 Lx 길이 생성
            anchor = pd.to_datetime(cfg.train_end_date)  # 예: 2024-06-15
            # dates = [anchor-Lx+1, ..., anchor]  (총 Lx개)
            start_date = anchor - pd.Timedelta(days=Lx - 1)
            dates = [start_date + pd.Timedelta(days=i) for i in range(Lx)]

            # 전 구간 0으로 채움 (shape: [Lx, n_items])
    ...
    # ---- 입력 텐서 ----
    x_np = np.ascontiguousarray(values[-Lx:, :].T)  # [B, L]
    if cfg.log1p:
...
if __name__ == "__main__":
    ...
    if len(test_files) == 0:
        ...
        for test_idx, test_file in enumerate(test_files):
            ...
            df_preds = {}
            for name in model_names:
                df = predict_one_file_generic(cfg, trained_models[name], tdf, ds.store2idx, ds.cat2idx, ds.type2idx, ds.cluster2idx, menu_meta=menu_meta)
                if df.empty:
                    print(f"  ⚠️ {name} 예측이 비었습니다. 스킵.")
                df_preds[name] = df
            ...
            # 사용 가능한 모델만 가중 재정규화
            active = [name for name in model_names if (df_preds.get(name) is not None and not df_preds[name].empty)]
            w_vec = np.array([weights[name] for name in active], dtype=np.float64)
            if w_vec.sum() <= 0:
            ...
            for name, wv in zip(active, w_vec):
                mix += wv * df_preds[name].values

...
class EnhancedNHiTSConfig:
    ...
    use_hwise_prior: bool = True
    prior_tau: float = 1.2
    prior_blend: float = 0.5

    use_p_calib: bool = True
    gamma_grid_lo: float = 0.85
    gamma_grid_hi: float = 1.15
    gamma_grid_n: int = 13

    use_recent_bias: bool = True
    bias_winsor_p: float = 0.01

    artifacts_dir: str = "artifacts"
    stacker_artifact: str = "artifacts/stacker.pkl"
...
def _smape_np(y_true: np.ndarray, y_pred: np.ndarray, eps: float = 1e-6) -> float:
    denom = (np.abs(y_true) + np.abs(y_pred)).clip(eps, None)
    return np.mean(2.0 * np.abs(y_pred - y_true) / denom)


def _smape_by_model_h(y_true: np.ndarray, y_pred_m: np.ndarray) -> np.ndarray:
    """
    y_true: [N, H]
    y_pred_m: [M, N, H]
    return: [M, H] sMAPE per model & horizon
    """
    M, N, H = y_pred_m.shape
    out = np.zeros((M, H), dtype=np.float64)
    for m in range(M):
        for h in range(H):
            out[m, h] = _smape_np(y_true[:, h], y_pred_m[m, :, h])
    return out
...
class StackerConfig:
    use_stacker: bool = True
    stacker_type: str = "ridge+"  # options: 'ridge+'(positive ridge)
    use_hwise_prior: bool = True
    prior_tau: float = 1.2        # softmax temperature for H-wise prior
    prior_blend: float = 0.5      # 0~1, portion of H-wise prior in blend with global w
    ...
    use_recent_bias: bool = True
    bias_winsor_p: float = 0.01   # winsorize residuals when computing bias per item
...
class OOFStacker:
    """
    Fit on OOF predictions to learn ensemble weights and simple calibrations.

    After fit():
      - self.w: [M] global non-negative weights (sum=1)
      - self.w_h: [H, M] horizon-wise blended weights (if use_hwise_prior)
      - self.gamma: scalar for p-calibration (if use_p_calib)
      - self.bias_by_item: dict(item_name -> float) mean residual (if use_recent_bias)
    """
    def __init__(self, cfg: StackerConfig):
        self.cfg = cfg
        self.model_names: List[str] = []
        self.w: Optional[np.ndarray] = None
        self.w_h: Optional[np.ndarray] = None
        self.gamma: Optional[float] = None
        self.bias_by_item: Optional[Dict[str, float]] = None
        self.H: Optional[int] = None

    # ------------------------------
    # core learners
    # ------------------------------
    def _fit_weights(self, yh_oof: np.ndarray, yt_oof: np.ndarray, model_names: List[str]) -> np.ndarray:
        """
        yh_oof: [N, M, H]; yt_oof: [N, H]
        returns w: [M]
        """
        N, M, H = yh_oof.shape
        X = yh_oof.reshape(N * H, M)
        y = yt_oof.reshape(N * H)

        if not _HAS_SK:
            # safe fallback: inverse-loss weights
            smapes = []
            for m in range(M):
                smapes.append(_smape_np(y, X[:, m]))
            smapes = np.array(smapes)
            inv = 1.0 / np.clip(smapes, 1e-6, None)
            w = inv / (inv.sum() + 1e-12)
            return w.astype(np.float64)

        reg = Ridge(alpha=1e-6, fit_intercept=False, positive=True)
        reg.fit(X, y)
        w = np.maximum(0.0, reg.coef_.astype(np.float64))
        s = w.sum()
        if s <= 0:
            w[:] = 1.0 / len(w)
        else:
    ...
    def _fit_hwise_prior(self, yt_oof: np.ndarray, yh_oof: np.ndarray, w_base: np.ndarray) -> np.ndarray:
        N, M, H = yh_oof.shape
        # compute per-model per-horizon sMAPE
        y_pred_m = np.transpose(yh_oof, (1, 0, 2))  # [M, N, H]
        loss_mh = _smape_by_model_h(yt_oof, y_pred_m)  # [M, H]
        w_h = np.zeros((H, M), dtype=np.float64)
        for h in range(H):
            score = -loss_mh[:, h] / max(1e-8, self.cfg.prior_tau)
            prior = _softmax(score, axis=0)  # [M]
            w_h[h] = (1.0 - self.cfg.prior_blend) * w_base + self.cfg.prior_blend * prior
            # renormalize for safety
            s = w_h[h].sum()
            w_h[h] = w_h[h] / (s + 1e-12)
        return w_h

    def _fit_gamma(self, p_oof: Optional[np.ndarray], yt_oof: np.ndarray) -> Optional[float]:
        if (not self.cfg.use_p_calib) or (p_oof is None):
            return None
        # z_true: zero/positive indicator
        z_true = (yt_oof > 0).astype(np.float64)
        grid = np.linspace(self.cfg.gamma_grid_lo, self.cfg.gamma_grid_hi, self.cfg.gamma_grid_n)
        best, gbest = 1e18, 1.0
        for g in grid:
            ph = np.clip(g * p_oof, 0.0, 1.0)
            # simple surrogate error on zero/positive classification
            err = np.mean(np.abs(ph - z_true))
            if err < best:
                best, gbest = err, g
        return float(gbest)

    def _fit_bias(self, yt_oof: np.ndarray, yh_oof: np.ndarray, item_names_oof: Optional[List[str]]) -> Optional[Dict[str, float]]:
        if (not self.cfg.use_recent_bias) or (item_names_oof is None):
            return None
        # residual per sample: mean over horizon
        res = yh_oof.mean(axis=(1, 2)) - yt_oof.mean(axis=1)  # [N]
        res = _winsorize(res, self.cfg.bias_winsor_p)
        bias: Dict[str, List[float]] = {}
        for r, name in zip(res, item_names_oof):
            bias.setdefault(name, []).append(float(r))
        out = {k: float(np.mean(v)) for k, v in bias.items()}
        return out
    ...
    def fit(
        self,
        yh_oof: np.ndarray,             # [N, M, H] final predictions per model (value*prob)
        yt_oof: np.ndarray,             # [N, H]
        model_names: List[str],
        p_oof: Optional[np.ndarray] = None,   # [N, M, H] or [N, H] if single prob; we accept [N,H] as mean prob over models
        item_names_oof: Optional[List[str]] = None,
    ) -> "OOFStacker":
        assert yh_oof.ndim == 3 and yt_oof.ndim == 2, "Shapes must be [N,M,H] and [N,H]"
        N, M, H = yh_oof.shape
        self.model_names = list(model_names)
        self.H = H

        # 1) global weights
        w = self._fit_weights(yh_oof, yt_oof, self.model_names)
        self.w = w

        # 2) H-wise prior
        if self.cfg.use_hwise_prior:
            self.w_h = self._fit_hwise_prior(yt_oof, yh_oof, w)
        else:
            self.w_h = None

        # 3) gamma (if p_oof provided)
        if p_oof is not None and p_oof.ndim == 3:
            # average prob across models (robust)
            p_mean = p_oof.mean(axis=1)
        else:
            p_mean = p_oof  # could be [N,H] or None
        self.gamma = self._fit_gamma(p_mean, yt_oof)

        # 4) bias per item
        self.bias_by_item = self._fit_bias(yt_oof, yh_oof, item_names_oof)
        return self
    ...
    def save(self, path: str) -> None:
        ...
        blob = {
            ...
            "w_h": None if self.w_h is None else self.w_h.tolist(),
            "gamma": self.gamma,
    ...
    @staticmethod
    def load(path: str) -> "OOFStacker":
        import json
        with open(path, "r", encoding="utf-8") as f:
            blob = json.load(f)
        cfg = StackerConfig()
        s = OOFStacker(cfg)
        s.model_names = blob.get("model_names", [])
        w = blob.get("w")
        w_h = blob.get("w_h")
        s.w = None if w is None else np.array(w, dtype=np.float64)
        s.w_h = None if w_h is None else np.array(w_h, dtype=np.float64)
        s.gamma = blob.get("gamma")
        s.bias_by_item = blob.get("bias_by_item")
        s.H = blob.get("H")
        return s
    ...
    # apply
    # ------------------------------
    def combine(self, yh_test: np.ndarray) -> np.ndarray:
        """
        yh_test: [B, M, H] final model predictions (already prob*value)
        return: [B, H]
        """
        assert yh_test.ndim == 3
        B, M, H = yh_test.shape
        if self.w_h is not None and self.w_h.shape == (H, M):
            out = np.zeros((B, H), dtype=np.float64)
            for h in range(H):
                w_h = self.w_h[h]  # [M]
                out[:, h] = np.tensordot(yh_test[:, :, h], w_h, axes=([1], [0]))
            return out
        # fallback to global w
        assert self.w is not None, "Weights are not fitted"
        return np.tensordot(yh_test, self.w, axes=([1], [0]))

    def apply_gamma(self, p: np.ndarray) -> np.ndarray:
        if self.gamma is None:
            return p
        return np.clip(self.gamma * p, 0.0, 1.0)

    def apply_bias(self, y_hat: np.ndarray, item_names: Optional[List[str]]) -> np.ndarray:
        if not self.bias_by_item or not item_names:
            return y_hat
        out = y_hat.copy()
        for i, it in enumerate(item_names):
            b = self.bias_by_item.get(it)
            if b is not None:
                out[i, :] = np.maximum(0.0, out[i, :] - float(b))
        return out
...
class EnhancedNHiTSModel(nn.Module):
    ...
    def forward(
        self,
        x: torch.Tensor,                  # [B, L]
        past_cal: torch.Tensor,           # [B, L, cal_dim]
        fut_cal: torch.Tensor,            # [B, H, cal_dim]
        store_idx: torch.Tensor,          # [B]
        cat_idx: torch.Tensor,            # [B]
        type_idx: torch.Tensor,           # [B]
        weekend_ratio: torch.Tensor = None,
        **kwargs
    ):
        # 메타 피처 준비
        ...
        for i, block in enumerate(self.blocks):
            ...
            if self.stack_types[i] == "trend":
                ...
                block_output = F.interpolate(
                    ...
                    mode=self.interpolation_mode, **kwargs
                ).squeeze(1)  # [B, out_len]
...
class GenericTrainer:
    ...
    def predict_on_loader(self, loader: DataLoader):
        ...
        yh_list, yt_list, pp_list, idx_list = [], [], [], []
        device = self.device
        use_amp = bool(self.cfg.use_amp and torch.cuda.is_available() and device.type == "cuda")
        amp_ctx = torch.autocast(device_type="cuda", dtype=torch.float16) if use_amp else torch.cuda.amp.autocast(enabled=False)
        ...
        try:
            with amp_ctx:
                for batch in loader:
                    ...
                    weekend_ratio = batch.get("weekend_ratio", None)
                    if isinstance(weekend_ratio, torch.Tensor):
                        weekend_ratio = weekend_ratio.to(device)
                    else:
                        weekend_ratio = None
                    ...
                    v_log, p_logits = self.model(
                        x, past_cal, fut_cal, si, ci, ti,
                        weekend_ratio=weekend_ratio, **extra
                    )

                    y_val = torch.expm1(ylog).clamp_min(0).cpu().numpy()
                    y_hat = (torch.sigmoid(p_logits) * torch.expm1(v_log)).clamp_min(0).cpu().numpy()
                    p_sig = torch.sigmoid(p_logits).cpu().numpy()

                    yh_list.append(y_hat)
                    yt_list.append(y_val)
                    pp_list.append(p_sig)

                    if "item_idx" in batch:
                        idx_list.append(batch["item_idx"].cpu().numpy())
                    else:
                        B = y_val.shape[0]
                        idx_list.append(np.arange(B))
        finally:
            # 3) 어떤 일이 있어도 원래 가중치로 복구
        ...
        if yh_list:
            ...
            pp = np.concatenate(pp_list, axis=0)  # [N,H]
            idxs = np.concatenate(idx_list, axis=0)
        else:
            yh = yt = pp = idxs = np.array([])

        return yt, yh, pp, idxs
            ...
            for batch in train_loader:
                ...
                with amp_ctx:
                    ...
                    v_pred, p_logits = self.model(
                        ...
                        weekend_ratio=weekend_ratio, **batch_dict
                    )
...
def _smape_np(y_true: np.ndarray, y_pred: np.ndarray, eps: float = 1e-6) -> np.ndarray:
    denom = (np.abs(y_true) + np.abs(y_pred)).clip(eps, None)
    sm = 2.0 * np.abs(y_true - y_pred) / denom
    return sm.mean(axis=0)  # [H]

def _smape_np_hwise(y_true: np.ndarray, y_pred: np.ndarray, eps: float = 1e-6) -> np.ndarray:
    denom = (np.abs(y_true) + np.abs(y_pred)).clip(eps, None)
    return (2.0 * np.abs(y_true - y_pred) / denom).mean(axis=0)  # [H]

@torch.no_grad()
def eval_hwise_smape(trainer, loader) -> np.ndarray:
    trainer.model.eval()
    device = trainer.device
    yh, yt = [], []
    use_amp = bool(trainer.cfg.use_amp and torch.cuda.is_available() and device.type == "cuda")
    amp = torch.autocast(device_type="cuda", dtype=torch.float16) if use_amp else torch.cuda.amp.autocast(enabled=False)
    with amp:
        for batch in loader:
            x = batch["x"].to(device)
            ylog = batch["y"].to(device)
            ...
            excluded = {"x","y","past_cal","fut_cal","store_idx","cat_idx","type_idx","sample_w","pos_mask","weekend_ratio"}
            extra = {k:(v.to(device) if isinstance(v, torch.Tensor) else v) for k,v in batch.items() if k not in excluded}

            v_log, p_logits = trainer.model(x, past_cal, fut_cal, si, ci, ti, weekend_ratio=weekend_ratio, **extra)
            y_true = torch.expm1(ylog).clamp_min(0).cpu().numpy()
            y_hat  = (torch.sigmoid(p_logits) * torch.expm1(v_log)).clamp_min(0).cpu().numpy()
            yt.append(y_true); yh.append(y_hat)
    yt = np.concatenate(yt, axis=0)  # [N,H]
    yh = np.concatenate(yh, axis=0)  # [N,H]
    return _smape_np_hwise(yt, yh)  # [H]    # [H], “낮을수록” 좋은 loss

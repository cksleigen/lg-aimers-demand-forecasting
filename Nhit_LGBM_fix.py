...
class EnhancedNHiTSConfig:
    ...
    # ★ Trial 15 고정 하이퍼파라미터 (학습 스케줄 포함)
    EPOCHS_FULL: int = 150
    BATCH_FULL: int = 256
    BASE_LR_FULL: float = 8.766069282311903e-04       # base_lr
    MAX_LR_FULL: float = 1.8882243739191584e-03       # max_lr
    WD_FULL: float      = 1.5029935747077396e-04      # weight_decay

    # Optuna 비활성화 (고정값 사용)
    USE_OPTUNA: bool = False
    N_TRIALS: int = 50          # 사용 안 함
    EPOCHS_TUNE: int = 50       # 사용 안 함
    BATCH_TUNE: int = 256       # 사용 안 함
    ...
    dropout: float = 0.11253128050295698

    # 스택/다운샘플 (동적 생성 그대로 사용)
...
class EnhancedNHiTSTrainer:
    ...
    def train_with_loaders(self, train_loader: DataLoader, val_loader: DataLoader):
        ...
        self.sched = torch.optim.lr_scheduler.OneCycleLR(
            self.optim, max_lr=self.max_lr,
            epochs=self.epochs, steps_per_epoch=len(train_loader),
            pct_start=0.05, div_factor=self.max_lr / self.base_lr
        )
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            # ★ 둘 다 출력
            print(f"[Epoch {epoch:03d}] train_loss: {train_loss_avg:.5f}  val_loss: {val_loss:.5f}")
...
def train_lgbm_oof(dataset: EnhancedNHiTSDataset, mask_val: np.ndarray):
    ...
    params = dict(
        ...
        num_leaves=63, feature_fraction=0.9, bagging_fraction=0.8,
        bagging_freq=1, min_data_in_leaf=20, max_depth=-1, verbose=-1
    )

    callbacks = [
        lgb.early_stopping(stopping_rounds=100, verbose=False),
        lgb.log_evaluation(period=0),  # 로깅 끔(원하면 50 등으로 바꿔)
    ]
    ...
    for h in range(dataset.cfg.out_len):
        ...
        dva = lgb.Dataset(
            X_va[h], label=y_va[h],
            reference=dtr, feature_name=feat_names, free_raw_data=False
        )
        ...
        model = lgb.train(
            ...
            valid_sets=[dtr, dva],          # ← 리스트로 전달
            valid_names=["tr", "va"],       # ← 이름은 여기서 지정
            callbacks=callbacks,
        )
        models.append(model)

        # best_iteration 안전 처리
        best_it = model.best_iteration if model.best_iteration is not None else model.current_iteration()
        va_pred = model.predict(X_va[h], num_iteration=best_it)

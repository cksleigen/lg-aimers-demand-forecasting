# -*- coding: utf-8 -*-
"""model5.py_0819.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/199floqmmL-1_W6g-sYdrzfMYuPIyVccw
"""
...
class EnhancedNHiTSConfig:
    # 경로
    train_csv: str = "/content/drive/MyDrive/data/train/train.csv"
    ...
    out_submission_csv: str = "/content/drive/MyDrive/data/0819_feature_added_final_5model_submission.csv"
...
def create_regular_holiday_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_copy['is_holiday_regular'] = 0
    cond_lagrotta_23 = (df_copy['영업장명'] == '라그로타') & (df_copy['연도'] == 2023) & (df_copy['월'].isin(range(2, 12))) & (df_copy['요일'] == 'Monday')
    cond_lagrotta_24 = (df_copy['영업장명'] == '라그로타') & (df_copy['연도'] == 2024) & (df_copy['월'].isin(range(3, 7))) & (df_copy['요일'] == 'Monday')
    df_copy.loc[cond_lagrotta_23 | cond_lagrotta_24, 'is_holiday_regular'] = 1
    cond_forestreet_23_03 = (df_copy['영업장명'] == '포레스트릿') & (df_copy['연도'] == 2023) & (df_copy['월'] == 3)
    cond_forestreet_23_09 = (df_copy['영업장명'] == '포레스트릿') & (df_copy['연도'] == 2023) & (df_copy['월'] == 9) & (df_copy['요일'].isin(['Monday', 'Tuesday', 'Wednesday']))
    cond_forestreet_24_03 = (df_copy['영업장명'] == '포레스트릿') & (df_copy['연도'] == 2024) & (df_copy['월'] == 3)
    cond_forestreet_23_05_onwards = (df_copy['영업장명'] == '포레스트릿') & (df_copy['영업일자'] >= '2023-05-01') & (df_copy['요일'].isin(['Monday', 'Tuesday', 'Wednesday', 'Thursday']))
    df_copy.loc[cond_forestreet_23_03 | cond_forestreet_23_09 | cond_forestreet_24_03 | cond_forestreet_23_05_onwards, 'is_holiday_regular'] = 1
    stores_hwadam = ['화담숲주막', '화담숲카페']
    cond_hwadam_winter = (df_copy['영업장명'].isin(stores_hwadam)) & (df_copy['월'].isin([12, 1, 2, 3]))
    cond_hwadam_rest_mon = (df_copy['영업장명'].isin(stores_hwadam)) & (~df_copy['월'].isin([12, 1, 2, 3])) & (df_copy['요일'] == 'Monday')
    df_copy.loc[cond_hwadam_winter | cond_hwadam_rest_mon, 'is_holiday_regular'] = 1
...
def get_longest_off_season_feature(df: pd.DataFrame) -> pd.DataFrame:
    """
    모든 메뉴의 가장 긴 비판매 기간을 계산하고, 90일 이상인 메뉴를
    'is_long_off_season' 피처로 반환.
    """
    df_copy = df.copy()
    df_copy['영업일자'] = pd.to_datetime(df_copy['영업일자'])
    ...
    off_season_df = pd.DataFrame(longest_zero_streak.items(), columns=['영업장명_메뉴명', 'longest_off_season_days'])

    # 📌 90일 기준을 반영하여 'is_long_off_season' 이진 피처 추가
...
class EnhancedNHiTSDataset(Dataset):
    def __init__(self, cfg: EnhancedNHiTSConfig, df: pd.DataFrame):
        ...
        if 'longest_off_season_days' not in self.df.columns:
            raise ValueError("'longest_off_season_days' 컬럼이 데이터프레임에 없습니다.")
        # 📌 수정된 부분: is_holiday_regular 컬럼 존재 여부 확인
        if 'is_holiday_regular' not in self.df.columns:
            raise ValueError("'is_holiday_regular' 컬럼이 데이터프레임에 없습니다.")
        ...
        # 📌 수정된 부분: national holidays만 holidays_set으로 전달
        holidays_set_national = set(pd.to_datetime(cfg.custom_holidays_list)) if cfg.custom_holidays_list else set()

        caldf = build_enhanced_features(self.dates, holidays_set_national)
        self.cal_feats = caldf.drop(columns=["date"]).values.astype(np.float32)
        self.cal_feat_names = [c for c in caldf.columns if c != "date"]

        # 📌 수정된 부분: is_holiday_regular 피처도 caldf에 추가
        regular_holidays_pivot = self.df.pivot(index=cfg.date_col, columns=cfg.item_col, values='is_holiday_regular').sort_index().fillna(0)
        self.is_holiday_regular_feats = regular_holidays_pivot.values.astype(np.float32)

        item_features = self.df.groupby(cfg.item_col)['longest_off_season_days'].first().reindex(self.items)
        self.item_offseason_days = item_features.values.astype(np.float32)
    ...
    def __getitem__(self, idx):
        ...
        # 📌 수정된 부분: 정기 휴무 피처를 추가
        past_reg_hol = self.is_holiday_regular_feats[t0:t0 + Lx, j]
        fut_reg_hol = self.is_holiday_regular_feats[t0 + Lx:t0 + Lx + Ly, j]

        store_idx = self.item_store_idx[j]
        cat_idx = self.item_cat_idx[j]
        type_idx = self.item_type_idx[j]
        sample_w = self.sample_weights[j]
        offseason_days = self.item_offseason_days[j]
        ...
        return {
            ...
            "past_reg_hol": torch.from_numpy(past_reg_hol).float(),
            "fut_reg_hol": torch.from_numpy(fut_reg_hol).float(),
            ...
            "offseason_days": torch.tensor(offseason_days, dtype=torch.float32),
...
if __name__ == "__main__":
    ...
    # 📌 2. 메뉴별 피처 (longest_off_season_days) 생성 및 병합
    print("\n[단계 1/3] 비판매 기간 피처 생성 및 병합 중...")
    off_season_feature = get_longest_off_season_feature(train_df)
    train_df = pd.merge(train_df, off_season_feature, on='영업장명_메뉴명', how='left')

    # 📌 3. 매장 정기 휴무일 피처 생성 및 병합
    print("[단계 2/3] 매장 정기 휴무일 피처 생성 및 병합 중...")
    train_df = create_regular_holiday_features(train_df)

    # 📌 4. EnhancedNHiTSDataset 클래스 초기화
    print("\n[단계 3/3] Dataset 클래스 초기화...")
    ds = EnhancedNHiTSDataset(cfg, train_df)

    print("\n✅ 모든 피처가 포함된 데이터셋 클래스가 성공적으로 초기화되었습니다.")

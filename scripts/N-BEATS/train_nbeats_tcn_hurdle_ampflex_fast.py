...
# -*- coding: utf-8 -*-
"""
train_nbeats_tcn_hurdle_ampflex.py  (FAST-1H ready)
...
- Optuna (optional), EMA, OneCycleLR
- FAST PRESET (--fast1h): 튜닝 스킵, 최신 폴드 1개, 윈도우 다운샘플링, BF16 권장
...
# ---------- FAST: 윈도우 다운샘플링 ----------
def downsample_windows(windows: List[WindowRecord], stride: int = 1, max_per_item: int = 0):
    if stride <= 1 and max_per_item <= 0:
        return windows
    by_item = {}
    for w in windows:
        by_item.setdefault(w.item_name, []).append(w)
    reduced = []
    for item, lst in by_item.items():
        lst.sort(key=lambda x: x.last_input_date)
        lst2 = lst[::max(1, stride)]
        if max_per_item > 0 and len(lst2) > max_per_item:
            lst2 = lst2[-max_per_item:]
        reduced.extend(lst2)
    return reduced
...
class WinDataset(Dataset):
    def __init__(self, ws: List[WindowRecord], id2store: Dict[int,str]):
        self.ws = ws
        self.id2store = id2store
    ...
    def __getitem__(self, i):
        ...
        sw = torch.tensor(STORE_WEIGHTS.get(self.id2store.get(w.store_id,''), 1.0), dtype=DTYPE)
...
def train_one_fold(nmodel, tmodel, cfg: TrainConfig, train_ws, val_ws, id2store):
    ...
    try:
        torch.set_float32_matmul_precision('high')  # TF32 경로 가속(Pytorch>=2.0, A6000 OK)
    except Exception:
        pass
    ...
    for epoch in range(cfg.epochs):
        ...
        for batch in train_loader:
            ...
            if use_scaler:
                ...
                torch.nn.utils.clip_grad_norm_(params, 0.5)  # 안정성
...
def parse_args():
    ...
    ap.add_argument("--amp_dtype", type=str, default="fp16", choices=["fp16","bf16"])  # AMP 전환
    ap.add_argument("--num_workers", type=int, default=4)

    # FAST-1H 프리셋
    ap.add_argument("--fast1h", action="store_true",
                    help="1시간 내 학습 스피드 프리셋: 튜닝 스킵, 최신 폴드 1개, 윈도우 다운샘플링")
    ap.add_argument("--window_stride", type=int, default=1,
                    help="윈도우 샘플링 보폭(>=1). 2면 절반만 사용")
    ap.add_argument("--max_windows_per_item", type=int, default=0,
                    help="품목당 학습 윈도우 상한(0=무제한). 최근 윈도우 위주로 잘라 속도↑")
    ap.add_argument("--batch_size", type=int, default=0,
                    help="0이면 최적 파라미터의 batch_size 사용, >0이면 강제 적용")
...
def main(args):
    ...
    if args.fast1h:
        before = len(windows)
        windows = downsample_windows(windows, stride=args.window_stride, max_per_item=args.max_windows_per_item)
        print(f"[FAST] windows: {before} -> {len(windows)} (stride={args.window_stride}, max/item={args.max_windows_per_item})")
    ...
    # 5) 고정 최적 하이퍼파라미터 (사용자 제공)
    fixed_best = {
      "n_hidden": 512, "n_blocks": 2, "n_dropout": 0.09633208794129916, "emb_store": 48,
      "use_menu": False, "emb_menu": 0, "n_cond": 128,
      "t_channels": 256, "t_levels": 3, "t_kernel": 5, "t_dropout": 0.14761412739096127,
      "t_emb_store": 32, "t_use_menu": True, "t_emb_menu": 16, "t_cond": 32,
      "lambda_pos": 0.19998808410540156, "blend_alpha": 0.43512791425412073,
      "lr": 0.001499350467459821, "wd": 0.000003702678391250746, "batch_size": 2048
    }
    # --batch_size 인자로 강제 변경 가능
    if args.batch_size and args.batch_size > 0:
        fixed_best["batch_size"] = args.batch_size
    
    # ------------------------------------------------------
    # --- ensure all required keys exist for FixedTrial ---
    fixed_best.setdefault("emb_cat", 16)
    fixed_best.setdefault("emb_type", 8)
    fixed_best.setdefault("t_emb_cat", 16)
    fixed_best.setdefault("t_emb_type", 8)

    # use_menu=False면 emb_menu=0 고정 (이미 0이면 그대로 둠)
    if not fixed_best.get("use_menu", False):
        fixed_best["emb_menu"] = 0
    # ------------------------------------------------------

    # 6) Optuna (선택) — fast1h면 무조건 스킵하고 fixed_best 사용
    best_trial_params = None
    if not args.fast1h and not args.inference_only:
        def objective(trial):
        ...
        best_trial_params = fixed_best
        args.inference_only = True  # 튜닝 강제 스킵

    # 7) 최종 모델 구성 & 최근 fold 기준 재학습
    ...
    tcfg.lr = best_trial_params.get("lr", tcfg.lr)
    tcfg.weight_decay = best_trial_params.get("wd", tcfg.weight_decay)
    tcfg.batch_size = best_trial_params.get("batch_size", tcfg.batch_size)
    tcfg.lambda_pos = best_trial_params.get("lambda_pos", tcfg.lambda_pos)

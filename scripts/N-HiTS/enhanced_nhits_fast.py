# -*- coding: utf-8 -*-
"""
Enhanced N-HiTS Model - 단독 최강 성능
- 목표: SMAPE 0.688 → 0.45 목표로 하지만 최종 스코어는 0.5205907194 나옴
...
class EnhancedNHiTSConfig:
    ...
    # RTX 4060 최적화 하이퍼파라미터
    EPOCHS_FULL: int = 120  # 150 → 120 (적당히 감소)
    BATCH_FULL: int = 768   # 512 → 768 (RTX 4060 VRAM 최적화)
    ...
    N_TRIALS: int = 25      # 50 → 25 (시간 단축)
    EPOCHS_TUNE: int = 35   # 50 → 35 (시간 단축)
    BATCH_TUNE: int = 512   # 256 → 512
    
    # CV 설정
    cv_fold_end_dates: Tuple[str, str, str] = ("2024-06-14", "2024-06-07", "2024-05-31")
    
    # RTX 4060 최적화 DataLoader
    num_workers: int = 6    # 8 → 6 (RTX 4060에 적합)
    ...
    n_blocks: int = 3
    n_layers: int = 2
    n_pool_kernel_size: List[int] = None  # [2, 2, 1] 기본값
    pooling_mode: str = "MaxPool1d"
    interpolation_mode: str = "linear"
    dropout: float = 0.1
    
    # Multi-scale 설정 (N-HiTS 핵심)
    stack_types: List[str] = None  # ["identity", "identity", "trend"] 기본값
    n_freq_downsample: List[int] = None  # [2, 1, 1] 기본값
...
class EnhancedNHiTSModel(nn.Module):
    ...
    def __init__(self, in_len: int, out_len: int, cal_dim: int, n_stores: int,
        ...
        # Hierarchical interpolation weights (안전한 접근)
        self.basis_weights = nn.ParameterList([
            nn.Parameter(torch.randn(out_len, max(1, out_len // max(1, cfg.n_freq_downsample[min(i, len(cfg.n_freq_downsample)-1)]))))
            for i in range(cfg.n_blocks)
        ])
    ...
    def forward(self, x, past_cal, fut_cal, store_idx, cat_idx, type_idx):
        ...
        for i, block in enumerate(self.blocks):
            ...
            # Hierarchical basis function 적용 (안전한 접근)
            stack_type = self.stack_types[min(i, len(self.stack_types)-1)]
            if stack_type == "trend":
                # Trend decomposition
                basis = self.basis_weights[i]  # [out_len, downsampled_len]
                # 간단한 선형 보간으로 trend 적용
                if basis.size(1) > 1:  # 안전한 크기 체크
...
def run_optuna(cfg: EnhancedNHiTSConfig):
    ...
    def objective(trial: optuna.trial.Trial):
        # N-HiTS 특화 하이퍼파라미터 (안전한 범위)
        cfg.hidden = trial.suggest_categorical("hidden", [384, 512, 640])
        cfg.n_blocks = trial.suggest_categorical("n_blocks", [2, 3])  # 4 제거 (안정성)  + 시간 단축을 위함.
...
if __name__ == "__main__":
    ...
    # 🚀 RTX 4060 빠른 모드 (선택사항)
    # True로 설정하여 모델학습 돌림.
    FAST_MODE = True # False  # True로 설정하면 1시간 내 완료
    if FAST_MODE:
        # print("⚡️ RTX 4060에 맞게 실행")
        cfg.USE_OPTUNA = False
        cfg.EPOCHS_FULL = 60
        cfg.BATCH_FULL = 1024 # 40-96 -> 2048 -> 1024로 줄임 

# -*- coding: utf-8 -*-
"""
Enhanced N-HiTS (All-Fix Edition for Competition)
- 반영 항목:
...
class EnhancedNHiTSConfig:
    ...
    out_submission_csv: str = "./data/double_enhanced_nhits_submission.csv"
...
def get_menu_category(menu_name: str) -> str:
    ...
    elif any(word in menu_lower for word in ['अ메리카노', '아메리카노', '라떼', '커피']):
        return 'coffee'
...
class EnhancedNHiTSModel(nn.Module):
    ...
    def __init__(self, in_len: int, out_len: int, cal_dim: int, n_stores: int,
        ...
        # 보조 발생확률(head) - 단, 최종 예측에 곱하지 않음 (학습 보조용)
...
class EnhancedNHiTSTrainer:
    ...
    def make_loaders_from_mask(self, mask_val: np.ndarray):
        ...
        # self.dataset.target_end_dates 는 numpy array(혹은 Timestamp 배열) 가정
        times_np = self.dataset.target_end_dates[train_idx]              # np.ndarray of Timestamps/datetime64[ns]
...
def evaluate_cfg_rolling(cfg: EnhancedNHiTSConfig, epochs: int, batch_size: int,
    ...
    for end_date_str in cfg.cv_fold_end_dates:
        ...
        print(f"[CV] fold end={end_date_str} val={best_val:.6f}")
    cv_mean = float(np.mean(fold_vals))
    print(f"[CV] mean val={cv_mean:.6f}")
    return cv_mean
...
def predict_one_file(cfg: EnhancedNHiTSConfig, model: EnhancedNHiTSModel, test_df: pd.DataFrame,
    ...
    model.eval()
    with amp_ctx:
        v_pred, p_logits = model(x, past_cal_b, fut_cal_b, store_idx, cat_idx, type_idx, item_idx)
...
if __name__ == "__main__":
    ...
    print(f"🎯 Final val loss (proxy for Weighted-SMAPE): {final_val_loss:.6f}")

...
def apply_best_params(cfg, best: dict):
    # 스칼라형
    cfg.hidden            = int(best.get("hidden", cfg.hidden))
    cfg.store_emb_dim     = int(best.get("store_emb_dim", cfg.store_emb_dim))
    cfg.dropout           = float(best.get("dropout", cfg.dropout))
    cfg.zero_weight       = float(best.get("zero_weight", cfg.zero_weight))
    cfg.hurdle_lambda     = float(best.get("hurdle_lambda", cfg.hurdle_lambda))
    cfg.use_menu_embedding = bool(best.get("use_menu_embedding", cfg.use_menu_embedding))
    if cfg.use_menu_embedding:
        cfg.menu_emb_dim  = int(best.get("menu_emb_dim", cfg.menu_emb_dim))

    # scales: 문자열/리스트/튜플 모두 안전 처리 -> 튜플[int]
    scales_val = best.get("scales", cfg.scales)
    if isinstance(scales_val, str):
        cfg.scales = tuple(int(x) for x in scales_val.split(","))
    elif isinstance(scales_val, (list, tuple)):
        cfg.scales = tuple(int(x) for x in scales_val)
    else:
        raise ValueError(f"Unexpected scales type: {type(scales_val)}")

    # 안전 가드: 부호/범위 sanity check (원하면 더 추가)
    assert all(isinstance(s, int) and s >= 1 for s in cfg.scales), "scales must be positive ints"
...
if __name__ == "__main__":
    ...
    best_params_from_log = {
        "hidden": 512,
        "store_emb_dim": 48,
        "dropout": 0.0818995508141358,
        "zero_weight": 0.05,
        "hurdle_lambda": 0.2,
        "scales": "1,2,4,7,14",
        "use_menu_embedding": False
        # "menu_emb_dim": 24  # 미사용이므로 굳이 필요 없음
    }
    apply_best_params(cfg, best_params_from_log)

    # --- 안전 가드 (중복이지만 한번 더: 문자열이면 튜플[int]) ---
    if isinstance(cfg.scales, str):
        cfg.scales = tuple(int(x) for x in cfg.scales.split(","))
    elif isinstance(cfg.scales, list):
        cfg.scales = tuple(int(x) for x in cfg.scales)

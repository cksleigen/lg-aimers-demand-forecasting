...
# --- (안전) parse_store_name 이 없으면 정의
try:
    parse_store_name  # type: ignore
except NameError:
...
class EnhancedNHiTSConfig:
    ...
    out_submission_csv: str = "LG_output/0816_enhanced_4model_nhits_submission.csv"
    ...
    zero_weight: float = 0.025
    hurdle_lambda: float = 0.08
...
def get_menu_category(menu_name: str) -> str:
    ...
    dd_kw      = ['아이스크림','식혜','콜라','스프라이트','에이드','아이스트']
...
class UltraEnhancedHurdleLoss(nn.Module):
    def __init__(self, eps: float = 0.01, zero_weight: float = 0.01, lambda_bce: float = 0.08):
        super().__init__()
...
class ImprovedTimesNetTiny(nn.Module):
    def __init__(self, cfg: EnhancedNHiTSConfig, in_len: int, out_len: int,
        ...
        self.blk7  = ImprovedTimesBlock(C, C, seq_len=in_len, kernels=(3,5,7), top_k=5, dropout=cfg.tnet_dropout)
        self.blk14 = ImprovedTimesBlock(C, C, seq_len=in_len, kernels=(3,5,7), top_k=5, dropout=cfg.tnet_dropout)
        self.blk28 = ImprovedTimesBlock(C, C, seq_len=in_len, kernels=(3,5,7), top_k=5, dropout=cfg.tnet_dropout)
...
class GRUTiny(nn.Module):
    def __init__(self, cfg: EnhancedNHiTSConfig, in_len: int, out_len: int,
        ...
        self.meta_gate = nn.Sequential(
            nn.Linear(cfg.gru_head_hidden, cfg.gru_head_hidden), nn.SiLU(),
            nn.Linear(cfg.gru_head_hidden, 1), nn.Sigmoid()
        )
        self.prob_head = nn.Sequential(
    ...
    def forward(self, x, past_cal, fut_cal, store_idx, cat_idx, type_idx, cluster_idx, mean, std, slope):
        ...
        meta_adj = self.meta_integration(meta_feat)
        gate = self.meta_gate(F.layer_norm(meta_adj, meta_adj.shape[1:]))  # [B,1]
...
class GenericTrainer:
    def __init__(self, cfg: EnhancedNHiTSConfig, dataset: EnhancedNHiTSDataset, model: nn.Module,
        ...
        self.scaler = torch.cuda.amp.GradScaler(enabled=bool(self.cfg.use_amp and torch.cuda.is_available() and self.device.type == "cuda"))
    ...
    def train_with_loaders(self, train_loader: DataLoader, val_loader: DataLoader, model_name="Model"):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            amp_enabled = self.scaler.is_enabled()  # CUDA+AMP일 때만 True
...
# ------------------------------
# A) 파일 1개 예측 (게이트 OFF 기본) + 확률행 반환 옵션
# ------------------------------
@torch.no_grad()
def predict_one_file_generic(
    ...
    use_prob_gate: bool = False,   # ✅ 기본 OFF
    return_prob: bool = False,     # 확률행 반환
):
    device = torch.device(cfg.device)
    if test_df is None or len(test_df) == 0:
        return (pd.DataFrame(), pd.DataFrame()) if return_prob else pd.DataFrame()
    ...
    try:
        pivot = (tdf.pivot(index=cfg.date_col, columns=cfg.item_col, values=cfg.target_col)
                   .sort_index()
                   .fillna(0.0))
    except Exception:
        return (pd.DataFrame(), pd.DataFrame()) if return_prob else pd.DataFrame()
    ...
    if len(dates) < Lx:
        ...
        if len(dates) > 0:
            ...
            values = np.vstack([np.repeat(ref, pad_rows, axis=0), values]).astype(np.float32, copy=False)
            start_date = dates[0]
            dates = [start_date - pd.Timedelta(days=i) for i in range(pad_rows, 0, -1)] + dates
        else:
            anchor = pd.to_datetime(cfg.train_end_date)
    ...
    def flags_for_dates(item: str, dts: List[pd.Timestamp]) -> np.ndarray:
        meta = (menu_meta or {}).get(str(item), {})
        launch_date = pd.to_datetime(meta.get("launch_date")) if meta.get("launch_date") else None
        discontinue_date = pd.to_datetime(meta.get("discontinue_date")) if meta.get("discontinue_date") else None
        active_months = set(int(m) for m in meta.get("active_months", []))
        out = np.zeros((len(dts), 3), dtype=np.float32)
        for i, dd in enumerate(dts):
            out[i,0] = 1.0 if (launch_date is not None and dd >= launch_date) else 0.0
            out[i,1] = 1.0 if (discontinue_date is not None and dd >  discontinue_date) else 0.0
            out[i,2] = 1.0 if (active_months and int(dd.month) in active_months) else 0.0
        return out

    x_np = np.ascontiguousarray(values[-Lx:, :].T)
    if cfg.log1p:
        x_np = np.log1p(x_np.clip(min=0.0))
    B = x_np.shape[0]
    if B == 0:
        return (pd.DataFrame(), pd.DataFrame()) if return_prob else pd.DataFrame()
    x = torch.from_numpy(x_np).to(device=device, dtype=torch.float32)

    past_dates = dates[-Lx:]; fut_dates = future_dates
    past_cals = []; fut_cals = []
    for it in items:
        past_cals.append(np.concatenate([past_cal_base, flags_for_dates(it, past_dates)], axis=1))
        fut_cals.append( np.concatenate([fut_cal_base,  flags_for_dates(it, fut_dates)],  axis=1))
    past_cal_b = torch.from_numpy(np.stack(past_cals, axis=0)).to(device=device, dtype=torch.float32)
    fut_cal_b  = torch.from_numpy(np.stack(fut_cals,  axis=0)).to(device=device, dtype=torch.float32)
    ...
    sdef = next(iter(store2idx.values()), 0)
    cdef = next(iter(cat2idx.values()), 0)
    tdef = next(iter(type2idx.values()), 0)
    kdef = next(iter(cluster2idx.values()), 0)

    store_idx   = torch.as_tensor([store2idx.get(s, sdef) for s in stores], device=device, dtype=torch.long)
    cat_idx     = torch.as_tensor([cat2idx.get(get_menu_category(m), cdef) for m in menus], device=device, dtype=torch.long)
    type_idx    = torch.as_tensor([type2idx.get(get_store_type(s), tdef) for s in stores], device=device, dtype=torch.long)
    cluster_idx = torch.as_tensor([cluster2idx.get(cluster_mapping.get(it, 0), kdef) for it in items], device=device, dtype=torch.long)

    mean  = x.mean(dim=1); std = x.std(dim=1); slope = (x[:, -1] - x[:, 0]) / max(1, Lx)
    ...
    model.eval()
    with amp_ctx:
        v_pred_log, p_logits = model(x, past_cal_b, fut_cal_b, store_idx, cat_idx, type_idx, cluster_idx, mean, std, slope)
        ...
        if use_prob_gate:
            ...
            y_prob = torch.sigmoid(p_logits)
            y_hat  = y_val
        y_hat = y_hat.clamp_min(0.0).detach().cpu().numpy()
        p_out = y_prob.detach().cpu().numpy()

    Y = pd.DataFrame(y_hat.T, index=[f"D+{i}" for i in range(1, cfg.out_len + 1)], columns=items).replace([np.inf,-np.inf],0.0).fillna(0.0)
    P = pd.DataFrame(p_out.T, index=Y.index, columns=Y.columns)
    return (Y, P) if return_prob else Y

# ------------------------------
# B) 한 파일 예측 앙상블(역손실 가중) + 확률 평균
# ------------------------------
def ensemble_predict_one_file(
    cfg: EnhancedNHiTSConfig,
    models: Dict[str, nn.Module],          # {"TimesNet": model, ...}
    test_df: pd.DataFrame,
    ds: EnhancedNHiTSDataset,
    menu_meta: Optional[Dict[str, Dict]],
    avg_val_losses: Dict[str, float],      # {"TimesNet": 0.33, ...}
    use_prob_gate: bool = False,
):
    names = [n for n in ["TimesNet","GRU","DLinear","N-HiTS","PatchTST"] if n in models]
    vals  = [avg_val_losses.get(n, np.nan) for n in names]
    w = invloss_weights(vals)
    Ys, Ps = [], []
    for n in names:
        Y, P = predict_one_file_generic(cfg, models[n], test_df, ds.store2idx, ds.cat2idx, ds.type2idx, ds.cluster2idx,
                                        menu_meta=menu_meta, use_prob_gate=use_prob_gate, return_prob=True)
        Ys.append(Y); Ps.append(P)
    # 값은 가중 평균, 확률은 단순 평균
    Y_ens = sum(wi * Yi for wi, Yi in zip(w, Ys))
    P_ens = sum(Ps) / len(Ps)
    return Y_ens, P_ens

# ------------------------------
# C) 가벼운 후처리: (i) 저확률 0-클램프, (ii) 스토어×Horizon 비율 보정(±10%)
# ------------------------------
def apply_postprocess(
    Y: pd.DataFrame,            # 값 예측 (rows: D+1..D+7, cols: item)
    P: Optional[pd.DataFrame],  # 확률행(같은 shape), 없으면 None
    tau_per_h: Optional[List[float]] = None,   # 예: [0.12,0.12,0.12,0.14,0.16,0.18,0.20]
    ratio_per_store_h: Optional[Dict[Tuple[str,int], float]] = None,  # ("카페테리아",1)->1.04
) -> pd.DataFrame:
    out = Y.copy()
    # (i) 저확률 0-클램프
    if P is not None and tau_per_h is not None:
        for h_idx, row in enumerate(out.index, start=1):
            if h_idx <= len(tau_per_h):
                tau = float(tau_per_h[h_idx-1])
                mask = P.loc[row] < tau
                out.loc[row, mask.index[mask]] = 0.0
    # (ii) 스토어×Horizon 비율 보정
    if ratio_per_store_h:
        for col in out.columns:
            store = parse_store_name(col)
            for h_idx, row in enumerate(out.index, start=1):
                r = ratio_per_store_h.get((store, h_idx), 1.0)
                r = float(np.clip(r, 0.90, 1.10))  # 과보정 방지
                out.loc[row, col] *= r
    return out

# ------------------------------
# D) 제출 전 가드(정렬/라운딩/음수캡핑) + 템플릿 순서 강제
# ------------------------------
def sanitize_predictions(df_pred: pd.DataFrame, item_cols: list,
                         clip_min: float = 0.0, round_to_int: bool = True) -> pd.DataFrame:
    out = df_pred.copy()
    for c in item_cols:
        vals = out[c].to_numpy()
...
def enforce_template_order(df_pred: pd.DataFrame, submission_template: pd.DataFrame,
                           date_col="영업일자", shop_col="식음업장명") -> pd.DataFrame:
    keep_cols = [shop_col, date_col] + [c for c in submission_template.columns if c not in [shop_col, date_col]]
    pred = df_pred.copy()
    tmpl_keys = submission_template[[shop_col, date_col]].drop_duplicates().sort_values([shop_col, date_col])
    if not set([shop_col, date_col]).issubset(pred.columns):
        # 필요 시 wide/long 변환 로직을 여기 보강
        pass
    pred = pred.merge(tmpl_keys, on=[shop_col, date_col], how="right")
    pred = pred.sort_values([shop_col, date_col]).reset_index(drop=True)
...
if __name__ == "__main__":
    ...
    # ===== 모델 학습 (멀티 폴드 평균 val) =====
    model_names = ["TimesNet", "GRU", "DLinear", "N-HiTS", "PatchTST"]  # SeasonalNaive 제거
    ...
    for name in model_names:
        ...
        for fold_i, end_date in enumerate(cfg.cv_fold_end_dates, start=1):
            ...
            trainer = GenericTrainer(cfg, ds, model_f, epochs=cfg.EPOCHS_FULL, batch_size=cfg.BATCH_FULL,
    ...
    if len(test_files) == 0:
        ...
        for test_idx, test_file in enumerate(test_files):
            ...
            try:
                ...
                continue
            # 1) 역손실 가중 앙상블 예측 (게이트 OFF) + 확률 반환
            Y, P = ensemble_predict_one_file(
                cfg, trained_models, tdf, ds, menu_meta, avg_val_losses,
                use_prob_gate=False
            )

            # 2) 가벼운 후처리 (저확률 0-클램프 등)
            Y2 = apply_postprocess(
                Y, P,
                tau_per_h=[0.12, 0.12, 0.12, 0.14, 0.16, 0.18, 0.20],
                ratio_per_store_h=None
            )

            # 3) 가드(음수 캡핑/라운딩). 컬럼 = 아이템(메뉴)들
            items = list(Y2.columns)
            submit_block = Y2.copy()
            submit_block = sanitize_predictions(submit_block, item_cols=items, clip_min=0.0, round_to_int=True)

            # 4) 기존 파이프라인과 동일하게 index를 테스트 라벨로 세팅

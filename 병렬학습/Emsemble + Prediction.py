# =============================================================================
# Notebook 3: 앙상블 및 최종 예측
# =============================================================================

# 기존 코드 전체 복사 (공통 모듈들)
# [여기에 기존 v6_menu_day-of-week_strength_feature.py의 모든 import, 클래스, 함수들 복사]
...
def load_trained_models_from_checkpoints(cfg, ds):
    """체크포인트에서 모든 학습된 모델 로드"""
    model_names = ["N-HiTS", "PatchTST", "TimesNet", "GRU", "MLinear", "TSMixer", "TCN"]
    trained_models = {}
    avg_val_losses = {}
    ...
    best_params_all["MLinear"] = {
        "base_lr": 0.0005377, "max_lr": 0.001667, "weight_decay": 4.5283e-05,
        "grad_clip": 0.3176, "pat_ratio": 0.1633, "ml_hidden_dim": 512,
        "ml_dropout": 0.2586, "ml_residual": True, "ml_layer_norm": False,
    }
    
    for name in model_names:
        # 결과 JSON 로드
        result_file = f"{checkpoint_dir}/{name}_result.json"
        checkpoint_file = f"{checkpoint_dir}/{name}_best_fold.pth"
        
        if os.path.exists(result_file) and os.path.exists(checkpoint_file):
            try:
                with open(result_file, 'r', encoding='utf-8') as f:
                    result = json.load(f)
                    avg_val_losses[name] = result['avg_val_loss']
                ...
                # 모델 로드
                model = build_final_model_with_best(
                    name, cfg_for_model, ds, 
                    best_params_all.get(name) if name == "MLinear" else None
                )
                model.load_state_dict(torch.load(checkpoint_file, map_location=cfg.device))
                model.to(cfg.device)
                trained_models[name] = model
                
                print(f"✅ {name} 로드 완료 (val_loss: {avg_val_losses[name]:.5f})")
                
            except Exception as e:
                print(f"⚠️ {name} 로드 중 오류: {e}")
        else:
            print(f"⚠️ {name} 체크포인트 또는 결과 파일을 찾을 수 없음")
            if not os.path.exists(result_file):
                print(f"   누락: {result_file}")
            if not os.path.exists(checkpoint_file):
                print(f"   누락: {checkpoint_file}")
    
    return trained_models, avg_val_losses

def show_ensemble_summary(avg_val_losses, weights):
    """앙상블 요약 정보 출력"""
    print("\n" + "="*60)
    print("📊 앙상블 모델 성능 및 가중치")
    print("="*60)
    
    # 성능순으로 정렬
    sorted_models = sorted(avg_val_losses.items(), key=lambda x: x[1])
    
    print(f"{'순위':<4} {'모델명':<12} {'Val Loss':<10} {'가중치':<8} {'상태'}")
    print("-" * 50)
    
    for i, (model_name, val_loss) in enumerate(sorted_models, 1):
        weight = weights.get(model_name, 0.0)
        medal = "🥇" if i == 1 else "🥈" if i == 2 else "🥉" if i == 3 else f"{i:2d}"
        print(f"{medal:<4} {model_name:<12} {val_loss:<10.5f} {weight:<8.3f} ✅")
    
    print("\n💡 해석:")
    print(f"   🏆 최고 성능: {sorted_models[0][0]} (val_loss: {sorted_models[0][1]:.5f})")
    print(f"   📊 평균 val_loss: {np.mean(list(avg_val_losses.values())):.5f}")
    print(f"   🎯 앙상블 효과: 개별 모델보다 더 안정적인 예측 기대")
...
if __name__ == "__main__":
    ...
    print("=== Notebook 3: 앙상블 및 최종 예측 시작 ===")

    # 데이터셋 로드
    if not os.path.exists(cfg.train_csv):
        raise FileNotFoundError(f"훈련 파일을 찾을 수 없습니다: {cfg.train_csv}")
    ...
    # 학습된 모델들 체크포인트에서 로드
    print("\n🔄 체크포인트에서 모델 로드 중...")
    trained_models, avg_val_losses = load_trained_models_from_checkpoints(cfg, ds)
    
    if len(trained_models) == 0:
        print("❌ 로드된 모델이 없습니다!")
        print("   먼저 이전 노트북들(1, 2, 3)을 실행하여 모델을 학습시키세요.")
        raise ValueError("로드된 모델이 없습니다!")
    
    print(f"\n✅ 총 {len(trained_models)}개 모델 로드 완료")

    # 앙상블 가중치 계산
    model_names = list(trained_models.keys())
    vals_in_order = [avg_val_losses.get(n, np.nan) for n in model_names]
    weights_arr = np.array(invloss_weights(vals_in_order), dtype=np.float64)
    weights = dict(zip(model_names, weights_arr))
    
    # 앙상블 요약 출력
    show_ensemble_summary(avg_val_losses, weights)

    # 테스트 파일 확인
    print("\n🔍 테스트 파일 확인 중...")
    test_files = sorted(glob.glob(cfg.test_glob))
    if len(test_files) == 0:
        print("⚠️ 테스트 파일이 없습니다. 예측을 건너뜁니다.")
        print("   앙상블 가중치만 확인하고 종료합니다.")
    else:
        ...
        for test_idx, test_file in enumerate(test_files):
            print(f"  📊 {test_file} 처리 중... ({test_idx+1}/{len(test_files)})")
            ...
            for name in model_names:
                ...
                try:
                    ...
                    df = predict_one_file_generic(
                        cfg, trained_models[name], tdf, train_df, ds,
            ...
            # 앙상블 계산
            if len(df_preds) == 0:
                print(f"  ⚠️ {test_file} 유효 예측이 없어 스킵")
                continue

            print(f"  🎯 사용 가능한 모델: {list(df_preds.keys())}")

            # 아이템 통일 (모든 모델이 같은 아이템 순서로)
            items = list(next(iter(df_preds.values())).columns)
            for name in list(df_preds.keys()):
                df_preds[name] = df_preds[name].reindex(columns=items, fill_value=0.0)

            # 가중 평균 계산
            models_available = list(df_preds.keys())
            w = np.array([weights.get(n, 0.0) for n in models_available])
            w_sum = w.sum()
            if w_sum > 0:
                w = w / w_sum
            else:
                w = np.ones(len(models_available)) / len(models_available)

            print(f"  ⚖️ 실제 사용 가중치: {dict(zip(models_available, w.round(3)))}")

            # 앙상블 예측 계산
            mix = np.zeros_like(df_preds[models_available[0]].values, dtype=np.float64)
            for i, name in enumerate(models_available):
                mix += w[i] * df_preds[name].values
            ...
            submit_block = pd.DataFrame(
                mix,
                index=[f"TEST_{test_idx:02d}+{k}일" for k in range(1, cfg.out_len + 1)],
                columns=items
            )
            
            # 평활화 적용 (horizon 간 급격한 변화 완화)
            submit_block = smooth_horizon_block(submit_block)
            all_preds.append(submit_block)
            
            # 예측 통계
            print(f"  📈 예측 통계: 평균={mix.mean():.2f}, 최대={mix.max():.0f}, 0인 비율={np.mean(mix==0):.3f}")

        # 최종 제출 파일 생성
        if len(all_preds) == 0:
            print("\n❌ 유효한 예측 결과가 없어 제출 파일을 생성하지 않았습니다.")
        else:
            print(f"\n📋 최종 제출 파일 생성 중...")
            print(f"  🔢 예측 블록 수: {len(all_preds)}")
            ...
            # 최종 통계
            print(f"\n📊 최종 제출 파일 통계:")
            print(f"  📐 형태: {final_submit.shape}")
            print(f"  📈 전체 평균: {final_submit[num_cols].values.mean():.2f}")
            print(f"  🔝 최대값: {final_submit[num_cols].values.max():,}")
            print(f"  🕳️ 0인 비율: {(final_submit[num_cols].values == 0).mean():.3f}")
            
            # 날짜별 통계
            daily_means = []
            for i in range(cfg.out_len):
                day_mask = final_submit["영업일자"].str.contains(f"+{i+1}일")
                if day_mask.any():
                    day_mean = final_submit.loc[day_mask, num_cols].values.mean()
                    daily_means.append(day_mean)
            
            if daily_means:
                print(f"  📅 일별 평균: D+1={daily_means[0]:.1f} → D+7={daily_means[-1]:.1f}")
            ...
            file_size = os.path.getsize(cfg.out_submission_csv) / 1024
            print(f"\n✅ 앙상블 예측 완료!")
            print(f"📁 최종 파일: {cfg.out_submission_csv} ({file_size:.1f} KB)")
            
            # 앙상블 효과 분석
            print(f"\n🎯 앙상블 효과 분석:")
            if len(models_available) > 1:
                print(f"  🤝 {len(models_available)}개 모델 결합")
                print(f"  📊 다양성: 개별 모델의 약점을 상호 보완")
                print(f"  🎲 안정성: 단일 모델보다 robust한 예측")
                
                # 각 모델의 기여도
                print(f"  💡 주요 기여 모델:")
                sorted_weights = sorted([(name, weights[name]) for name in models_available], 
                                      key=lambda x: x[1], reverse=True)
                for i, (name, weight) in enumerate(sorted_weights[:3]):
                    rank = "🥇" if i == 0 else "🥈" if i == 1 else "🥉"
                    print(f"     {rank} {name}: {weight:.3f} ({weight*100:.1f}%)")
            else:
                print(f"  ⚠️ 단일 모델만 사용됨: {models_available[0]}")

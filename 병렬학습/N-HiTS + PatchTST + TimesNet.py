# =============================================================================
# Notebook 1: N-HiTS + PatchTST + TimesNet 학습
# =============================================================================
...
if __name__ == "__main__":
    ...
    print("=== Notebook 1: N-HiTS + PatchTST + TimesNet 학습 시작 ===")
    ...
    # 📌 이 노트북에서 학습할 모델들만 선택
    model_names = ["N-HiTS", "PatchTST", "TimesNet"]
    trained_models: Dict[str, nn.Module] = {}
    avg_val_losses: Dict[str, float] = {}

    # MLinear 최적 파라미터 (기존 코드에서)
    best_params_all = {}
    ...
    for name in model_names:
        ...
        # 초기 가중치 스냅샷
        base_model = build_final_model_with_best(name, cfg_for_model, ds, None)
        ...
        for fold_i, end_date in enumerate(cfg.cv_fold_end_dates, start=1):
            ...
            model_f = build_final_model_with_best(name, cfg_for_model, ds, None)
        ...
        # 베스트 폴드 모델을 보관 + 저장
        final_model = build_final_model_with_best(name, cfg_for_model, ds, None)
        ...
        if best_fold_state is not None:
            ...
            # 결과 JSON도 저장 (앙상블에서 사용)
            result = {
                'model_name': name,
                'avg_val_loss': avg_val,
                'fold_losses': fold_vals,
                'best_fold': best_fold
            }
            result_path = os.path.join(cfg.checkpoint_dir, f"{name}_result.json")
            with open(result_path, "w", encoding="utf-8") as f:
                json.dump(result, f, indent=2, ensure_ascii=False)

    print("✅ Notebook 1 (N-HiTS + PatchTST) 학습 완료!")
    print("체크포인트가 저장되었습니다. 다음 노트북을 실행하세요.")